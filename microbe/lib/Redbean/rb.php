...
class RedBean_OODBBean implements IteratorAggregate, ArrayAccess {
	...
	public function recursiveExport( $properties = null ){
		$arr = empty($properties)? $this->properties:$properties;
		foreach($arr as $k=>$v) {
			if (is_array( $v ) ){
				$arr[ $k ] = $this->recursiveExport( $v );
				continue;			
			}
			if( is_object( $v ) && method_exists ( $v , "recursiveExport" ) ){
				$arr[$k] = $v->recursiveExport();
			}
...
abstract class RedBean_QueryWriter_AQueryWriter {
	...
	protected function insertRecord( $table, $insertcolumns, $insertvalues ) {
		...
		if (count($insertvalues)>0 && is_array($insertvalues[0]) && count($insertvalues[0])>0) {
			...
			$insertSQL = "INSERT IGNORE INTO $table ( $idfield, ".implode(",",$insertcolumns)." ) VALUES ";
			...
			$result = $this->adapter->getCell( "INSERT IGNORE INTO $table ($idfield) VALUES($default) $suffix");
		}
...
class RedBean_QueryWriter_SQLiteT extends RedBean_QueryWriter_AQueryWriter implements RedBean_QueryWriter {
	...
	public function widenColumn( $type, $column, $datatype ) {
		...
		$q[] = "INSERT IGNORE INTO tmp_backup SELECT * FROM `$table`;";
		$q[] = "DROP TABLE `$table`;";
		$q[] = "CREATE TABLE `$table` ( `$idfield` INTEGER PRIMARY KEY AUTOINCREMENT  $newTableDefStr  );";
		$q[] = "INSERT IGNORE INTO `$table` SELECT * FROM tmp_backup;";
